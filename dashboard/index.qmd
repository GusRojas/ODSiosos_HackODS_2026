...
```{python}
#| content: valuebox
#| title: "Información de datos nacionales"
#| icon: "database-fill"
#| color: "success"
"292"
```
...
# Interdependencia {data-icon="diagram-3-fill" scrolling="true"}
...
**Monitoreo y cobertura de datos en los ODS.** La siguiente figura representa el nivel de monitoreo y la disponibilidad de datos de los Objetivos de Desarrollo Sostenible en México, cada burbuja corresponde a un ODS; su tamaño indica el número total de indicadores asociados y su posición refleja la disponibilidad de datos y el nivel de seguimiento. Los ODS ubicados en la zona crítica, en la parte inferior izquierda, presentan menor cobertura de información y mayores limitaciones para su monitoreo. En conjunto, la figura permite identificar qué objetivos están mejor medidos y cuáles presentan rezagos, evidenciando desequilibrios en la generación de datos que afectan la capacidad de análisis y toma de decisiones.
...
**Red de interdependencias entre los Objetivos de Desarrollo Sostenible (ODS) en México.** La figura representa una red donde cada nodo corresponde a un ODS y cada arista indica la existencia de correlaciones altas (|r| ≥ 0.7) entre sus indicadores asociados. Las aristas en <span style="color:#FF6B6B;font-weight:700">rojo</span> representan **trade-offs**, es decir, relaciones en las que el avance en un objetivo se asocia con retrocesos en otro; mientras que las aristas en <span style="color:#4ECDC4;font-weight:700">verdes-azules</span> indican **sinergias**, donde los progresos son mutuamente reforzados. La estructura de la red permite identificar ODS altamente interconectados que funcionan como nodos sistémicos, cuyos cambios tienen efectos amplificados sobre el conjunto del sistema.


### ACTO 2 · El colapso de la salud pública {width="50%"}

<p align="justify"> **Análisis de indicadores críticos del sistema de salud (ODS 3)**. En esta sección se presentan los principales indicadores que reflejan el desempeño reciente del sistema de salud en México. A través de estos gráficos se analizan tendencias clave como la evolución de las enfermedades vectoriales, la mortalidad por enfermedades crónicas y la cobertura de servicios médicos, con el objetivo de identificar señales de deterioro y áreas donde el sistema aún muestra capacidad de respuesta.
...
# Salud {data-icon="heart-fill" scrolling="true"}
...
if not vector_s.empty:
    fig_inf_s.add_trace(go.Bar(
        x=vector_s['periodo'], y=vector_s['valor'], name='Enfermedades vectoriales',
...
```{python}
#| content: valuebox
#| title: "Mortalidad por enfermedades crónicas"
...
El sistema de salud pública en México **no colapsó de golpe**; se deterioró en silencio, indicador tras indicador, año tras año. Cuatro señales lo evidencian: la caída sostenida en la cobertura de vacunación; el repunte de enfermedades que se creían controladas; el avance constante de las enfermedades crónicas; y el deterioro progresivo de la salud mental, muchas veces invisible.

### El contrapeso — lo que sí mejoró {width="50%"}

La narrativa honesta también reconoce los avances. La mortalidad materna disminuyó *60% desde 1990* y la mortalidad infantil se redujo a menos de la mitad. Además, el personal sanitario ha crecido de forma sostenida. Comprender dónde el sistema funcionó es tan crucial como reconocer dónde falló.
...
En 2015, México alcanzó su **pico histórico**: 96.3% de cobertura vacunal, resultado de décadas de política pública sostenida. Nueve años después, la cobertura disminuyó a 76.5%, situándose **por debajo del umbral mínimo de la OMS** (95%) necesario para garantizar la inmunidad de rebaño. Esta caída no responde a una sola causa, sino a la convergencia de factores como recortes presupuestales, el impacto de la pandemia y una creciente pérdida de confianza institucional, que en conjunto revirtieron avances construidos durante años.

### Alarma 2 · Las enfermedades que regresaron {width="50%"}

El dengue, el chikungunya y el zika habían sido contenidos en México. Sin embargo, en la última década, **las enfermedades transmitidas por vectores se dispararon**: un aumento de +619% entre 2017 y 2024. Al mismo tiempo, la tuberculosis, que alcanzó mínimos históricos en 2020, repuntó un 50% en apenas cuatro años. Dos retrocesos simultáneos que evidencian un debilitamiento en el sistema de vigilancia epidemiológica.
...
```{python}
#| title: "Cobertura de vacunación a través del tiempo"
fig_vacc_s.show()
```

```{python}
#| title: "Enfermedades infecciosas a traves del tiempo"
fig_inf_s.show()
```
...
Las enfermedades cardiovasculares, el cáncer, la diabetes y las enfermedades respiratorias crónicas provocan **272 muertes por cada 100,000 habitantes**, lo que representa un aumento del 22% respecto a 1998. El sobrepeso y la obesidad, presentes en el 39.1% de la población adulta, actúan como el principal factor estructural que impulsa esta tendencia.

### Alarma 4 · La crisis de salud mental invisible {width="50%"}

La tasa de suicidio en México se **triplicó en 30 años**, al pasar de 2.29 por cada 100,000 habitantes en 1990 a 6.17 en 2020. La tendencia ha sido sostenida y sin cambios significativos, lo que evidencia la ausencia de políticas de salud mental con impacto real. Además, los incrementos se acentúan en contextos de crisis económicas, lo que sugiere que el sistema no ha logrado responder de forma efectiva a estos periodos de mayor vulnerabilidad.
...
México presenta avances reales en salud que no deben pasarse por alto. La mortalidad materna se redujo de 85 a 35 por cada 100,000 nacidos vivos, mientras que la mortalidad infantil descendió de 41 a 16 por cada 1,000 nacidos vivos. Además, más del 88% de los partos cuentan con asistencia médica. Estos resultados evidencian que cuando existe continuidad institucional, financiamiento sostenido y capacidad operativa, el sistema responde.

### ODS 3 como nodo sistémico {width="50%"}

El ODS de salud se configura como el nodo más interconectado del sistema, con correlaciones relevantes con la mayoría de los demás objetivos. Cuando la salud se deteriora, sus efectos se propagan al resto del sistema. La relación más crítica se observa con el ODS de trabajo, dando lugar al trilema Salud–Trabajo–Agua que estructura gran parte del análisis.
...
# Trilema {data-icon="droplet-fill" scrolling="true"}
...
# Desigualdad {data-icon="scale" scrolling="true"}
...
El **índice de Gini**, que mide la desigualdad en la distribución del ingreso, bajó de 0.449 en 2016 a 0.391 en 2024, lo que refleja una mejora real en términos de equidad. Sin embargo, si se excluyen las transferencias del gobierno, el Gini se mantiene en 0.450. Esta diferencia muestra que la reducción de la desigualdad no proviene de cambios estructurales en la economía, sino de mecanismos de redistribución. En otras palabras, el sistema depende de sus propios correctivos para compensar las desigualdades que genera.
...
La pobreza extrema disminuyó del 18.3% al 5.9%, un logro histórico. Pero el 43.5% de los mexicanos sigue en pobreza según la definición nacional (2022). El 36.3% en pobreza multidimensional. Estas son tres maneras de medir el mismo problema, y ninguna cuenta una historia de triunfo completo.
...
Las mujeres ocupan el **48.2%** de la Cámara de Diputados uno de los avances más notables del sistema ODS mexicano, sin embargo, la brecha salarial persiste: **~0.90 pesos por cada peso masculino**. Y las mujeres realizan en promedio **7.6 horas semanales más** de trabajo doméstico no remunerado.

### La discriminación: lo que los números no alcanzan a ver {width="50%"}

Te lo dejo mucho más claro y directo:

El indicador ODS 10 mide la discriminación, pero casi no hay datos. Solo se registró en 2017 y en 2022. En ese tiempo aumentó de 20.2% a 23.7%, lo que indica que el problema creció. Sin embargo, no sabemos qué pasó en los años intermedios ni si la tendencia continúa. Esto muestra un problema más grande: el sistema no mide de forma constante uno de los temas más importantes, y cuando algo no se mide bien, no se puede entender ni atender.
...
> *"México tiene 11 indicadores para medir la desigualdad. Solo 4 tienen datos suficientes para análisis estadístico."*

# Educación {data-icon="book-fill" scrolling="true"}
...
El titular tapa el fondo. México celebra el **99.2%** de alfabetización pero esa cifra oculta dos crisis. La matrícula de educación superior apenas alcanza el **43.8%**, 21 puntos por debajo del promedio OCDE. Y solo el **37%** de los adultos participa en formación continua, sin recuperación post-COVID.

### La brecha digital en el salón de clases {width="50%"}

Electricidad llega al **95.6%** de las escuelas. Internet solo al **46.7%** y las computadoras en funcionamiento es aprox. **47.6%**, menos que en 2018 (54.3%)**. La pandemia aceleró la necesidad de conectividad digital justo cuando el sistema la retrocedía. La brecha no es técnica: es estructural.
...
# Monitoreo {data-icon="eye-fill" scrolling="true"}
...
De los 17 ODS, solo 8 tienen el **100% de sus indicadores** con algún dato. El ODS 1 (Pobreza) es el peor: apenas el **58%** de sus indicadores reporta datos. ODS 5 (Género) tiene 93% de cobertura pero **0% con ≥15 observaciones** presencia nominal pero ausencia analítica.

### El hambre en cámara lenta {width="50%"}

En 18 años, el **stunting infantil**, retraso en el crecimiento de niñas y niños causado por desnutrición crónica, pasó de 15.4% a 15.1%, prácticamente sin cambios. Al mismo tiempo, la inseguridad alimentaria moderada o severa afecta al 16.2% de la población en 2022, es decir, 1 de cada 6 personas. A esto se suma que el rendimiento del maíz se mantiene un 33% por debajo del promedio mundial, lo que refleja limitaciones persistentes en la capacidad productiva del sistema alimentario.
...
México tiene **11,592 km de litoral**, una de las mayores extensiones costeras del mundo, y sin embargo registra únicamente **4 indicadores ODS** para monitorear todo su ecosistema marino. El avance es tangible, las áreas marinas protegidas alcanzaron el 22.5%, pero el presupuesto destinado a estudiar ese mar se redujo **−78%** desde 2012, lo que evidencia una brecha entre conservación declarada y capacidad real de conocimiento y seguimiento.
